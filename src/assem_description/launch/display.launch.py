...
def generate_launch_description():
    assem_description_dir = get_package_share_directory("assem_description")

    # تحديد مسار ملف الـ xacro الأساسي
    model_arg = DeclareLaunchArgument(name="model", default_value=os.path.join(
                                        assem_description_dir, "urdf", "assem.urdf.xacro"
                                        ),
    ...
    rviz_node = Node(
        ...
        name="rviz2",
        output="screen",
        arguments=["-d", os.path.join(assem_description_dir, "rviz", "display.rviz")],
    )
